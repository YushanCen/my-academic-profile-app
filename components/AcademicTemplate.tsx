...
const AcademicTemplate: React.FC<AcademicTemplateProps> = ({ 
  ...
  // --- 新增辅助函数：智能处理行距 ---
  const formatLineHeight = (value?: string | number) => {
    if (value === undefined || value === null || value === '') return '1.6'; // 默认行距设为 1.6，阅读更舒适
    const str = String(value).trim();
    const num = Number(str);
    
    // 如果是纯数字
    if (!isNaN(num)) {
      // 如果数字小于 4 (比如 1.5, 2.0)，通常代表倍数，不加单位
      if (num < 4) return str;
      // 如果数字很大 (比如 24)，通常代表像素，加上 px
      return `${str}px`;
    }
    return str;
  };
  ...
  const getStyle = (style?: ElementStyle): React.CSSProperties => {
    if (!style) return { whiteSpace: 'pre-wrap', lineHeight: '1.6' };
    ...
    // --- 这里是修改的核心 ---
    // 我们在这个 return 对象里增加了 lineHeight 的处理
    return {
      fontSize: formatSize(style.fontSize),
      lineHeight: formatLineHeight(style.lineHeight), // <--- 新增这一行
